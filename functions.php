...
function besu_setup() {
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'besu' ),
		)
	);
...
function besu_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'besu_content_width', 640 );
}
...
function besu_scripts() {
	wp_enqueue_style( 'besu-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'besu-style', 'rtl', 'replace' );

	wp_enqueue_script( 'besu-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
